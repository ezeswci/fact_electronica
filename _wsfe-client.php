...
class WsFE
{
    ...
    private function SignTRA($certificado, $clave)
    {
		...
		fclose($fp);
		//----------------------
        $STATUS = openssl_pkcs7_sign("TRA.xml", "TRA.tmp", "file://" . $certificado,
            array("file://" . $clave, ""),
            array(),
            !PKCS7_DETACHED
        );
    ...
    function Autorizar($PtoVta, $TipoComp)
    {
        ...
        if (isset($results->FECAESolicitarResult->FeDetResp->FECAEDetResponse->Observaciones) and sizeof($results->FECAESolicitarResult->FeDetResp->FECAEDetResponse->Observaciones->Obs) > 0) {
